...
class solidProperties:
    ...
    def calculatePotentialEnergy(self):
        for i in range(len(self.particleLinVelocityList)):
            pe = self.particleMassList*np.dot(np.array([4.904999999999999, -8.495709211125343,0]),np.array([0.0,-0.7,0.0])-self.particlePositionList[i])
...
class particleWallContact:
    ...
    """
    def __init__(self,contactPair, point, normal):#sem vložit wallproperties
...
def main(filePath, maxNumberOfBodies):
    ...
    exporting("postProc-results/all_mechanical_energy", ["time", "mechanical_energy"], dem_cfd_times, mechEnergy)
