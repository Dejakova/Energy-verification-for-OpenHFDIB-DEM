...
class solidProperties:
            ...
            for j in body.particleParticleContactList:
                ...
                if j.cId == self.id or j.tId == self.id:          
                    for i in range(len(TimeValues[:-1])):
                        if(j.contactTime[0] == TimeValues[i]):
    ...
    def calculateMechanicEnergy(self,TimeValues, solidsInDomain):
        for i in range(len(self.allContactEnergy)):
...
class particleParticleContact:
    ...
    def adjust_dissipation_energy(self, TimeValues):
        ...
        for i in range(len(TimeValues[:-1])):
            if self.contactTime[0] == TimeValues[i]:
    ...
    def Allexport(self):
        ...
        exporting("postProc-results/contact_force_"+str(self.cId)+"-"+str(self.tId), ["time", "contact_force_"+str(self.cId)+"-"+str(self.tId)],  self.contactTime, self.contactForce)
        exporting("postProc-results/contact_dis_force_"+str(self.cId)+"-"+str(self.tId), ["time", "contact_dis_force_"+str(self.cId)+"-"+str(self.tId)],  self.contactTime, self.contactFNdclam)
...
def getInertiaData(data_chunk, solid, time):  
    string = solid.getRefenceStringInertia()
    for line in data_chunk:
        if line.startswith(string):
            extracted_data = [float(strToVal) for strToVal in re.findall(
                r"-?\ *[0-9]+\.?[0-9]*(?:[Ee]\ *[-+]?\ *[0-9]+)?", line[len(string):]
            )]
            solid.inertia.append(np.array(extracted_data))
            break


def getParticleParticleContactData(data_chunk, contactPair, current_time, previous_time, solidsInDomain):
    """ extract particle-particle contact data from a log chunk for a given contact pair
    
        appends to the corresponding contact object in the contact list of the particle
    """
    ...
    for i, line in enumerate(data_chunk):
        if referenceString in line and line.split(referenceString)[1].strip() == "":
            contactPrt = solidsInDomain[contactPair[0]].getActiveContactPair(contactPair[1], previous_time)

            if not contactPrt:
                solidsInDomain[contactPair[0]].particleParticleContactList.append(
                    particleParticleContact(contactPair)
                )
                contactPrt = solidsInDomain[contactPair[0]].particleParticleContactList[-1]

            contactPrt.contactTime.append(current_time)

            for j in range(i, len(data_chunk)):
                line_j = data_chunk[j]
                for k in stringList:
                    if line_j.startswith(k):
                        extracted_data = [float(val) for val in re.findall(
                            r"-?\ *[0-9]+\.?[0-9]*(?:[Ee]\ *[-+]?\ *[0-9]+)?", 
                            line_j[len(k):]
                        )]
                        if k == stringList[0]:
                            contactPrt.contactForce.append(np.linalg.norm(extracted_data))
                        elif k == stringList[1]:
                            contactPrt.contactNormal = np.array(extracted_data)
                        elif k == stringList[2]:
                            contactPrt.contactFNdclam.append(np.linalg.norm(extracted_data))
                        elif k == stringList[3]:
...
def getParticleWallContactData(data_chunk, contactPair, current_time, previous_time, solidsInDomain, wall):
    """ extract particle-wall contact data from a log chunk for a given contact pair.
    ...
    for i, line in enumerate(data_chunk):
        if referenceString in line and line.split(referenceString)[1].strip() == "":
            contactPrt = solidsInDomain[contactPair[0]].getActiveContactWall(contactPair[1], previous_time)

            if not contactPrt:
                solidsInDomain[contactPair[0]].particleWallContactList.append(
                    particleWallContact(contactPair, wall.point, wall.normal)
                )
                ...
                for k in stringList:
                    if line_j.startswith(k):
                        ...
                        if k == stringList[0]:
                            ...
                            contactPrt.contactWallFNdclam.append(np.linalg.norm(extracted_data))
                        elif k == stringList[3]:
                            contactPrt.contactVolume.append(extracted_data)
                        elif k == stringList[4]:
                            contactPrt.contactArea.append(extracted_data)
            ...
            if np.array_equal(contactPrt.normal, contactPrt.contactNormal):
                contactPrt.delta.append(getWallDelta(contactPrt))
            else:
                contactPrt.delta.append(0.0)
            break 

def read_lines_before_starting_time_loop(filePath):
    lines = []
...
def iterate_time_chunks(file_path):
    with open(file_path, 'r') as file:
        current_chunk = []
        current_time = None
        for line in file:
            if line.startswith("Time ="):
                if current_chunk and current_time is not None:
                    yield current_time, current_chunk
                    current_chunk = []
                try:
                    current_time = float(line.split("=")[-1].strip())
                except ValueError:
                    current_time = None
            elif line.startswith("updated HFDIBDEM"):
                if current_chunk and current_time is not None:
                    yield current_time, current_chunk
                    current_chunk = []
            else:
...
def extract_dem_cfd_subchunks(chunk):
    subchunks = []
    current_subchunk = []
    current_dem_cfd_time = None
    
    for line in chunk:
        if line.startswith(" DEM - CFD Time: "):
            if current_subchunk and current_dem_cfd_time is not None:
                subchunks.append((current_dem_cfd_time, current_subchunk))
                current_subchunk = []
            
            try:
                current_dem_cfd_time = float(line.split(":")[-1].strip())
            except ValueError:
                current_dem_cfd_time = None  
        else:
            if current_dem_cfd_time is not None:
...
def main(filePath, maxNumberOfBodies):
    initial_data = read_lines_before_starting_time_loop(filePath)
    bodyNames, bodiesStatic = getBodyNames(initial_data)
    nSolids, solidsInDomain = findBodiesInSimulation(initial_data, maxNumberOfBodies, bodyNames, bodiesStatic)
    ...
    for j in contactWallPermutation: #write wall properties
        wall0 = particleWallContact([j[0],1],np.array([0.05,0.04,0.0]),np.array([1.0,0.0,0.0]))
        wall1 = particleWallContact([j[0],2],np.array([0.0,0.0,0.0]),np.array([0.0,1.0,0.0]))
        wall2 = particleWallContact([j[0],3],np.array([0.0,0.0,0.0]),np.array([0.0,-1.0,0.0]))
        wall3 = particleWallContact([j[0],4],np.array([-0.05,0.04,0.0]),np.array([-1.0,0.0,0.0]))
        wall4 = particleWallContact([j[0],5],np.array([0.0,0.04,0.05]),np.array([0.0,0.0,1.0]))
        wall5 = particleWallContact([j[0],6],np.array([0.0,0.04,-0.05]),np.array([0.0,0.0,-1.0]))

        wallData=[wall0, wall1, wall2, wall3, wall4, wall5]
    
    main_times = []
    dem_cfd_times = []
    previous_main_time = None
    previous_dem_cfd_time = None
    
    #process each main time chunk
    for current_main_time, chunk in iterate_time_chunks(filePath):
        # print(f"Processing main time = {current_main_time}")
        if current_main_time not in main_times:
            main_times.append(current_main_time)

        for solid in solidsInDomain:
            getInertiaData(chunk, solid, current_main_time)
        
        dem_cfd_subchunks = extract_dem_cfd_subchunks(chunk)
        
        for dem_cfd_time, subchunk in dem_cfd_subchunks:
            # print(f"  Processing DEM-CFD time = {dem_cfd_time}")
            dem_cfd_times.append(dem_cfd_time)
            
            for j in solidsInDomain:
                getBodyData(subchunk, j, 0, len(subchunk), dem_cfd_time)
            
            if previous_dem_cfd_time is not None:
                for j in contactPermutation:
                    getParticleParticleContactData(subchunk, j, dem_cfd_time, previous_dem_cfd_time, solidsInDomain)
                
                for j in contactWallPermutation:
                    for wall in wallData:
                        if j[1] == wall.wallId:
                            getParticleWallContactData(subchunk, j, dem_cfd_time, previous_dem_cfd_time, solidsInDomain, wall)
            
            previous_dem_cfd_time = dem_cfd_time
        
        previous_main_time = current_main_time
    
    for body in solidsInDomain:
        #evaluate contact data for each body
        for contact in body.particleParticleContactList:
                contact.lastActive(dem_cfd_times)
                contact.calculateContactEnergy()
                contact.calculateDisEnergy()
                contact.adjust_dissipation_energy(dem_cfd_times)
                # contact.Allexport()
    ...
    for body in solidsInDomain:
        if body.static == False:
            ...
            body.calculateAllContactEnergy(dem_cfd_times, solidsInDomain)
            body.calculateRotationEnergy(main_times, dem_cfd_times)
            body.calculateMechanicEnergy(dem_cfd_times, solidsInDomain)
            body.calculateMomentum()
        else:
            body.calculateRotationEnergy(main_times, dem_cfd_times)
            exporting("postProc-results/linVelocity_body_"+str(body.id), ["time", "linVel_x_body_"+ str(body.id), "linVel_y_body_"+ str(body.id), "linVel_z_body_"+ str(body.id)], dem_cfd_times, body.particleLinVelocityList)
            exporting("postProc-results/position_body_"+str(body.id), ["time", "COM_x_body_"+ str(body.id), "COM_y_body_"+ str(body.id), "COM_z_body_"+ str(body.id)], dem_cfd_times, body.particlePositionList)
            exporting("postProc-results/angular_energy_"+str(body.id), ["time", "angular_energy_body_"+ str(body.id)], dem_cfd_times, body.angularEnergy)
            exporting("postProc-results/inertia_body_"+str(body.id), ["time", "inertia_"+ str(body.id)], main_times, body.inertia)
    ...
    exporting("postProc-results/all_mechanical_energy", ["time", "mechanical_energy"], dem_cfd_times, systemEnergy)
    exporting("postProc-results/all_kinetic_energy", ["time", "kinetic_energy"], dem_cfd_times, AllKinEnergy)
    exporting("postProc-results/all_potential_energy", ["time", "potential_energy"], dem_cfd_times, AllPotEnergy)
    exporting("postProc-results/all_contact_energy", ["time", "contact_energy"], dem_cfd_times, AllContactEnergy)
    exporting("postProc-results/all_angular_energy", ["time", "angular_energy"], dem_cfd_times, AllAngularEnergy)
    exporting("postProc-results/all_dis_energy", ["time", "dis_energy"], dem_cfd_times, AllDisEnergy)

main("log.HFDIBDEMFoam",110)