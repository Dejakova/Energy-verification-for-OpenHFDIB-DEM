...
def main(filePath, maxNumberOfBodies):
    ...
    for j in contactWallPermutation: #write wall properties
        wall0 = particleWallContact([j[0],1],np.array([0.1,0.04,0.0]),np.array([1.0,0.0,0.0]))
        wall1 = particleWallContact([j[0],2],np.array([0.0,0.5,0.0]),np.array([0.0,1.0,0.0]))
        wall2 = particleWallContact([j[0],3],np.array([0.0,0.0,0.0]),np.array([0.0,-1.0,0.0]))
        wall3 = particleWallContact([j[0],4],np.array([-0.1,0.04,0.0]),np.array([-1.0,0.0,0.0]))
    ...
    for body in solidsInDomain:
        if body.static == False:
            ...
            body.Allexport(dem_cfd_times, main_times)
        else:
            body.calculateRotationEnergy(main_times, dem_cfd_times)
