...
def exporting(path, header, time, data):
    with open(path, "w") as file:
        ...
        for i in range(0, len(data), 2):  
            row = [f"{time[i]:.18e}"]
            if columns == 1:
                row.append(f"{float(data[i]):.18e}")
            else:
                row.extend(f"{float(val):.18e}" for val in data[i][:columns])
            file.write(" ".join(row) + "\n")
...
class solidProperties:
    ...
    def calculateMechanicEnergy(self,TimeValues, solidsInDomain):
        for i in range(len(self.kinEnergy)):
            # e = self.kinEnergy[i] + self.potEnergy[i] + self.disEnergy[i] + self.angularEnergy[i] + self.allContactEnergy[i]   
