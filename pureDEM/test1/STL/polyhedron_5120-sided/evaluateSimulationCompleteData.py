...
class solidProperties:
    ...
    def calculatePotentialEnergy(self):
        for i in range(len(self.particleLinVelocityList)):
            # pe = self.particleMassList*np.dot(np.array([0,-9.81,0]),np.array([0.0,-0.015,0.0])-self.particlePositionList[i])
            pe = 0
    ...
    def calculateMechanicEnergy(self,TimeValues, solidsInDomain):
        for i in range(len(self.kinEnergy)):
            # print(len(self.kinEnergy))
            # print(len(self.potEnergy))
            # print(len(self.disEnergy))
            # print(len(self.allContactEnergy))
            # e = self.kinEnergy[i] + self.potEnergy[i] + self.disEnergy[i] + self.allContactEnergy[i]   
...
def countAmountContact(solidsInDomain):
    ...
    for body in solidsInDomain:
        ...
        for contact in body.particleParticleContactList:
            # print("body %d and body %d is %s"%(contact.cId, contact.tId, contact.active))
                ...
                for k in stringList:
                    ...
                    if f_ind == 0:
                        ...
                        if(k == stringList[0]):
                            ...
                            # print(np.linalg.norm(np.array(extracted_data)))
                        elif (k == stringList[3]):
...
def main(filePath, maxNumberOfBodies):
    ...
    for j in contactWallPermutation: #write wall properties
        wall0 = particleWallContact([j[0],1],np.array([0.015,0.0,0.0]),np.array([1.0,0.0,0.0]))
        wall1 = particleWallContact([j[0],2],np.array([0.0,0.05,0.0]),np.array([0.0,1.0,0.0]))
        wall2 = particleWallContact([j[0],3],np.array([0.0,-0.05,0.0]),np.array([0.0,-1.0,0.0]))
        wall3 = particleWallContact([j[0],4],np.array([-0.015,0.0,0.0]),np.array([-1.0,0.0,0.0]))
        wall4 = particleWallContact([j[0],5],np.array([0.0,0.0,0.015]),np.array([0.0,0.0,1.0]))
        wall5 = particleWallContact([j[0],6],np.array([0.0,0.0,-0.015]),np.array([0.0,0.0,-1.0]))
    ...
    for body in solidsInDomain:
        if body.static == False:
            ...
            # body.calculateDisEnergy(TimeValues, solidsInDomain)
            ...
            # body.calculateAbsEnergy()

            body.Allexport(TimeValues, CFDtime)
            
            # print(np.linalg.norm(body.particleLinVelocityList[-1]))
            # chyba=(np.linalg.norm(body.particleLinVelocityList[-1])-np.linalg.norm(body.particleLinVelocityList[0]))/np.linalg.norm(body.particleLinVelocityList[-1])*100
            # print("velocity error: %f"%abs(chyba))
        else:
            body.calculateRotationEnergy(CFDtime, TimeValues)
    ...
    exporting("postProc-results/all_dis_energy", ["time", "dis_energy"], TimeValues, AllDisEnergy)
    exporting("postProc-results/all_kinContact_energy", ["time", "kinContact_energy"], TimeValues, energy)

    # print(systemEnergy[-1])
    # print("energy error: %f"%abs((systemEnergy[-1]-systemEnergy[0])/systemEnergy[-1]*100))       
    
    # #--------- graphs for quick testing
    # plt.figure(figsize=(10,6))
    # plt.plot(TimeValues[:-1], systemEnergy, label = "mechanical energy", color = "black")
    # plt.plot(TimeValues[:-1], AllKinEnergy, label = "kinetic energy" , color = "yellow")
    # plt.plot(TimeValues[:-1], AllPotEnergy, label = "potential energy" , color = "blue")
    # plt.plot(TimeValues[:-1], energy1, label = "all energy", color = "red")
    # plt.plot(TimeValues[:-1], AllDisEnergy, label = "dissipative energy", color = "red")
    # plt.legend()
    # plt.xlabel("t (s)")
    # plt.ylabel("E (J)")
    # plt.title("numerical delta")
    # plt.savefig("difDelta_energies_new.png")
